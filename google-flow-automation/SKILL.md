...
*Never* use the instantaneous CLI `fill` command. Use `run-code` to simulate human typing with a randomized delay (30-100ms) between characters to avoid triggering "unusual activity" blocks.

```powershell
.\pw-bridge.bat run-code "async page => { const tb = page.getByRole('textbox').last(); await tb.click({ force: true }); const text = 'A photorealistic wolf howling at a full moon over a misty forest'; let currentText = ''; for (const char of text) { currentText += char; await tb.fill(currentText); const delay = Math.floor(Math.random() * 70) + 30; await page.waitForTimeout(delay); } }"
Start-Sleep -Seconds 2
```
...
```powershell
.\pw-bridge.bat run-code "async page => { const tb = page.getByRole('textbox').last(); await tb.click({ force: true }); const text = 'Your next prompt here'; let currentText = ''; for (const char of text) { currentText += char; await tb.fill(currentText); const delay = Math.floor(Math.random() * 70) + 30; await page.waitForTimeout(delay); } }"
Start-Sleep -Seconds 2
.\pw-bridge.bat click "button:has-text('Create') >> nth=1"
```
